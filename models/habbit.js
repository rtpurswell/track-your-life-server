...
const habbitSchema = mongoose.Schema({
    ...
    goal: {type: mongoose.Schema({
        frequency: {type:String, required:true, enum:["daily","monthly","weekly","biDaily","biWeekly"]},
...
const validate = (habbit) => {
    const schema =Joi.object({
        ...
        goal: Joi.object({
            frequency: Joi.string().required().valid("daily, weekly, monthly,biDaily,biWeekly"),

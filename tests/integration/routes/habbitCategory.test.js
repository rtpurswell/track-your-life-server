...
describe("/api/habbits/categories",()=>{
    ...
    describe("GET /", ()=>{
        const  exec =async () => {
            return await request(server).get("/api/habbits/categories").set(config.get("authTokenName"),token);
        }
    ...
    describe("GET /:id", ()=>{
        ...
        it("Should return 401 if the user does not provide a valid token", async ()=>{
            const res = await request(server).get(`/api/habbits/categories/${mongoose.Types.ObjectId()}`).set(config.get("authTokenName"),"");
            expect(res.status).toBe(401);
        });
        it("Should return 400 if the object Id is invalid", async ()=> {
            const res = await request(server).get(`/api/habbits/categories/1246`).set(config.get("authTokenName"),token);
        ...
        it("Should return 404 if the user is not the owner of the HabbitCategory", async ()=>{
            ...
            const res = await request(server).get(`/api/habbits/categories/${habbitCategory._id}`).set(config.get("authTokenName"),token);
        ...
        it("Should return 404 if it does not find an object with the given id",async ()=>{
           ...
           const res = await request(server).get(`/api/habbits/categories/${mongoose.Types.ObjectId()}`).set(config.get("authTokenName"),token);
           expect(res.status).toBe(404);
        });
    ...
    describe("POST /",()=>{
        ...
        const  exec =async () => {
            return await request(server).post("/api/habbits/categories").set(config.get("authTokenName"),token).send(habbitCategory);
        }
        it("Should return 401 if the user does not provide a valid token", async ()=>{
            token = "";
            const res = await exec();
        ...
        it("Should save HabbitCategory to database if it is valid and user is signed in", async ()=>{
            ...
            const habbitCategory = await HabbitCategory.findOne({userId:user._id, name:"Category 0"});
    ...
    describe("PUT /:id",()=>{
        ...
        const  exec =async () => {
            return await request(server).put(`/api/habbits/categories/${sampleHabbitCategory._id}`).set(config.get("authTokenName"),token).send(habbitCategory);
        }
        ...
        it("Should return 400 if Object ID is invalid", async ()=>{
            const res = await request(server).put(`/api/habbits/categories/1245`).set(config.get("authTokenName"),token).send(habbitCategory);
            expect(res.status).toBe(400);
        });
        it("Should return 404 if Object Id is not found", async ()=>{
            const res = await request(server).put(`/api/habbits/categories/${mongoose.Types.ObjectId()}`).set(config.get("authTokenName"),token).send(habbitCategory);
            expect(res.status).toBe(404);
        });
        ...
        it("Should update the existing HabbitCategory if input is valid and user owns it", async ()=> {
            ...
            const habbitCategorySearch = await HabbitCategory.findOne({userId:user._id, _id:sampleHabbitCategory._id, name:"Changed Name"});
    ...
    describe("DELETE /:id",()=>{
        ...
        const  exec =async () => {
            return await request(server).delete(`/api/habbits/categories/${sampleHabbitCategory._id}`).set(config.get("authTokenName"),token).send({replacedBy:replacedBy});
        }
        ...
        it("Should delete the record if the user owns it",async()=>{
            const res = await exec();
            const search = await HabbitCategory.findOne({_id:sampleHabbitCategory._id});

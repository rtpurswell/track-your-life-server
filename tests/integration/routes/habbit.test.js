...
describe("/api/habbit", () => {
      ...
      await Habbit.insertMany([
        {
          userId: user._id,
          name: "Test Habbit",
          description: "Test Habbit description",
  ...
  describe("GET /:id", () => {
    ...
    it("Should return a valid Habbit if the user is signed in and provided an existing Id", async () => {
      ...
      const res = await request(server)
        ...
        .set(config.get("authTokenName"), token);
      expect(res.status).toBe(200);
      expect(res.body.description).toBe("Test Habbit description");
    });
  });
  ...
  describe("POST /", () => {
    ...
    it("Should save Habbit to database if it is valid and user is signed in", async () => {
      ...
      const habbit = await Habbit.findOne({
        userId: user._id,
        name: "Test Habbit",
      });
      expect(habbit).not.toBeNull();
      expect(habbit.description).toBe("Test Habbit description");
    });
  });
  ...
  describe("PUT /:id", () => {
    ...
    const exec = async () => {
      return await request(server)
        .put(`/api/habbits/${sampleHabbit._id}`)
        .set(config.get("authTokenName"), token)
        .send(habbit);
    };
    ...
    it("Should return 404 if Object Id is not found", async () => {
      const res = await request(server)
        .put(`/api/habbits/${mongoose.Types.ObjectId()}`)
        .set(config.get("authTokenName"), token)
        .send(habbit);
      expect(res.status).toBe(404);
    });
    ...
    it("Should update the existing Habbit if input is valid and user owns it", async () => {
      ...
      const habbitSearch = await Habbit.findOne({
        userId: user._id,
        _id: sampleHabbit._id,
        name: "Changed Name",
      });
  ...
  describe("PATCH /:id", () => {
    ...
    beforeEach(async () => {
      ...
      await sampleHabbit.save();
      delete habbit.color;
      delete habbit.description;
      delete habbit.name;
    });

    const exec = async () => {
      return await request(server)
        .patch(`/api/habbits/${sampleHabbit._id}`)
        .set(config.get("authTokenName"), token)
        .send(habbit);
    };
    ...
    it("Should return 404 if Object Id is not found", async () => {
      const res = await request(server)
        .patch(`/api/habbits/${mongoose.Types.ObjectId()}`)
        .set(config.get("authTokenName"), token)
        .send(habbit);
      expect(res.status).toBe(404);
    });
  ...
  describe("DELETE /:id", () => {
    ...
    const exec = async () => {
      return await request(server)
        .delete(`/api/habbits/${sampleHabbit._id}`)
        .set(config.get("authTokenName"), token);
    };

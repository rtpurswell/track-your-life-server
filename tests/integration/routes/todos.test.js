...
describe("/api/todos", () => {
  ...
  describe("GET /:id ", () => {
    it("Should return 401 if the user does not provide a valid token", async () => {
      const res = await request(server)
        .get(`/api/todos/${mongoose.Types.ObjectId()}`)
  ...
  describe("POST / ", () => {
    ...
    it("Should save TodoBoard to database if it is valid and user is signed in", async () => {
      const res = await exec();
      const todoBoard = await TodoBoard.findOne({
        userId: user._id,
        _id: res.body._id,
      });
  ...
  describe("PUT /:id ", () => {
    ...
    const exec = async () => {
      return await request(server)
        .put(`/api/todos/${sampleTodoBoard._id}`)
        .set(config.get("authTokenName"), token)
        .send(todoBoard);
    };
  ...
  describe("DELETE /:id ", () => {
    ...
    const exec = async () => {
      return await request(server)
        .delete(`/api/todos/${sampleTodoBoard._id}`)
        .set(config.get("authTokenName"), token);
    };

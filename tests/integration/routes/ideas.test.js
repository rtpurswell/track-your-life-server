...
describe("/api/ideas",()=>{
    ...
    describe("GET /", ()=>{
        const  exec =async () => {
            return await request(server).get("/api/ideas").set(config.get("authTokenName"),token);
        }
    ...
    describe("GET /:id", ()=>{
        ...
        it("Should return 401 if the user does not provide a valid token", async ()=>{
            const res = await request(server).get(`/api/ideas/${mongoose.Types.ObjectId()}`).set(config.get("authTokenName"),"");
            expect(res.status).toBe(401);
        });
        it("Should return 400 if the object Id is invalid", async ()=> {
            const res = await request(server).get(`/api/ideas/1246`).set(config.get("authTokenName"),token);
        ...
        it("Should return 404 if the user is not the owner of the IdeaBoard", async ()=>{
            ...
            const res = await request(server).get(`/api/ideas/${ideaBoard._id}`).set(config.get("authTokenName"),token);
        ...
        it("Should return 404 if it does not find an object with the given id",async ()=>{
           ...
           const res = await request(server).get(`/api/ideas/${mongoose.Types.ObjectId()}`).set(config.get("authTokenName"),token);
           expect(res.status).toBe(404);
        });
    ...
    describe("POST /",()=>{
        ...
        const  exec =async () => {
            return await request(server).post("/api/ideas").set(config.get("authTokenName"),token).send(ideaBoard);
        }
        it("Should return 401 if the user does not provide a valid token", async ()=>{
            token = "";
            const res = await exec();
        ...
        it("Should save IdeaBoard to database if it is valid and user is signed in", async ()=>{
            ...
            const ideaBoard = await IdeaBoard.findOne({userId:user._id, name:"Idea Board 2"});
    ...
    describe("PUT /:id",()=>{
        ...
        const  exec =async () => {
            return await request(server).put(`/api/ideas/${sampleIdeaBoard._id}`).set(config.get("authTokenName"),token).send(ideaBoard);
        }
        ...
        it("Should return 400 if Object ID is invalid", async ()=>{
            const res = await request(server).put(`/api/ideas/1245`).set(config.get("authTokenName"),token).send(ideaBoard);
            expect(res.status).toBe(400);
        });
        it("Should return 404 if Object Id is not found", async ()=>{
            const res = await request(server).put(`/api/ideas/${mongoose.Types.ObjectId()}`).set(config.get("authTokenName"),token).send(ideaBoard);
            expect(res.status).toBe(404);
        });
        ...
        it("Should update the existing IdeaBoard if input is valid and user owns it", async ()=> {
            ...
            const ideaBoardSearch = await IdeaBoard.findOne({userId:user._id, _id:sampleIdeaBoard._id, name:"Changed Name"});
    ...
    describe("DELETE /:id",()=>{
        ...
        const  exec =async () => {
            return await request(server).delete(`/api/ideas/${sampleIdeaBoard._id}`).set(config.get("authTokenName"),token);
        }

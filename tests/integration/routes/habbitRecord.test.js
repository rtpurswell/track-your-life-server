...
describe("/api/habbits/records",()=>{
    ...
    describe("GET / ",()=>{
        const  exec =async () => {
            return await request(server).get("/api/habbits/records").set(config.get("authTokenName"),token);
        }
        ...
        it("Should return 400 if the startDate is invalid",async()=>{
            const res = await request(server).get("/api/habbits/records").set(config.get("authTokenName"),token).query({startDate:"asdfasdf",endDate:"2020-06-20"});
            expect(res.status).toBe(400);
        });
        it("Should return 400 if the endDate is invalid",async()=>{
            const res = await request(server).get("/api/habbits/records").set(config.get("authTokenName"),token).query({startDate:"2020-05-20",endDate:"asdfasdf"});
            expect(res.status).toBe(400);
        ...
        it("Should only return the habbit records that is within the date range",async()=>{
            ...
            const res = await request(server).get("/api/habbits/records").set(config.get("authTokenName"),token).query({startDate:"2020-05-20",endDate:"2020-06-20"});
    ...
    describe("GET /:id", ()=>{
        ...
        it("Should return 401 if the user does not provide a valid token", async ()=>{
            const res = await request(server).get(`/api/habbits/records/${mongoose.Types.ObjectId()}`).set(config.get("authTokenName"),"");
            expect(res.status).toBe(401);
        });
        it("Should return 400 if the object Id is invalid", async ()=> {
            const res = await request(server).get(`/api/habbits/records/1246`).set(config.get("authTokenName"),token);
            expect(res.status).toBe(400);
        ...
        it("Should return 404 if the user is not the owner of the Habbit Note", async ()=>{
            ...
            const res = await request(server).get(`/api/habbits/records/${habbitRecord._id}`).set(config.get("authTokenName"),token);
        ...
        it("Should return 404 if it does not find an object with the given id",async ()=>{
           ...
           const res = await request(server).get(`/api/habbits/records/${mongoose.Types.ObjectId()}`).set(config.get("authTokenName"),token);
           expect(res.status).toBe(404);
        });
    ...
    describe("POST /",()=>{
        ...
        const  exec =async () => {
            return await request(server).post("/api/habbits/records").set(config.get("authTokenName"),token).send(habbitRecord);
        }
        it("Should return 401 if the user does not provide a valid token", async ()=>{
            token = "";
            const res = await exec();
        ...
        it("Should save HabbitRecord to database if it is valid and user is signed in", async ()=>{
            ...
            const habbitRecord = await HabbitRecord.findOne({userId:user._id, _id:res.body._id});
    ...
    describe("PUT /:id",()=>{
        ...
        const  exec =async () => {
            return await request(server).put(`/api/habbits/records/${sampleHabbitRecord._id}`).set(config.get("authTokenName"),token).send(habbitRecord);
        }
        ...
        it("Should return 400 if Object ID is invalid", async ()=>{
            const res = await request(server).put(`/api/habbits/records/1245`).set(config.get("authTokenName"),token).send(habbitRecord);
            expect(res.status).toBe(400);
        });
        it("Should return 404 if Object Id is not found", async ()=>{
            const res = await request(server).put(`/api/habbits/records/${mongoose.Types.ObjectId()}`).set(config.get("authTokenName"),token).send(habbitRecord);
            expect(res.status).toBe(404);
        });
        ...
        it("Should update the existing HabbitRecord if input is valid and user owns it", async ()=> {
            ...
            const res = await exec();
            const habbitRecordSearch = await HabbitRecord.findOne({userId:user._id, _id:sampleHabbitRecord._id, date:"2021-01-01"});
    ...
    describe("DELETE /:id",()=>{
        ...
        const  exec =async () => {
            return await request(server).delete(`/api/habbits/records/${sampleHabbitRecord._id}`).set(config.get("authTokenName"),token);
        }
        ...
        it("Should delete the record if the user owns it",async()=>{
            ...
            const search = await HabbitRecord.findOne({_id:sampleHabbitRecord._id});

...
describe("USERS AND AUTH",()=>{
    describe("/api/auth",()=>{
        beforeEach(async()=>{
            ...
            const user = new User({name:"Test User", email:"jake@test.com", password:password});
            await user.save();
        });
        afterEach(async()=>{
            await User.remove();
        ...
        describe("POST /", ()=>{
            ...
            it("Should return 400 if the request is not valid",async()=>{
                requestBody = {email:"jake@test.com",password:"12345678",extra:"asdfasdf"};
            ...
            it("Should return a 200 response with a JWT in the header",async()=>{
                ...
                try {
                    const decoded = jwt.verify(res.header[config.get("authTokenName")], config.get("jwtPrivateKey"));
    ...
    describe("/api/users", ()=>{
        ...
        describe("POST /", ()=>{
            ...
            const exec = async()=>{return await request(server).post("/api/users").send(user)};
        ...
        describe("/api/users/validate",()=>{
            ...
            describe("GET / ",()=>{
                it("Should return 404 is the user doesnt exist",async()=>{
                    const res = await request(server).get("/api/users/validate").query({code:code, email:"jake@test.commmm"});
                    expect(res.status).toBe(404);
                });
                it("Should return 400 is the code is invalid",async()=>{
                    const res = await request(server).get("/api/users/validate").query({code:"asdfasdfasdf", email:"jake@test.com"});
                    expect(res.status).toBe(400);
                });
                it("Should validate the user if their code is correct.",async()=>{
                    code = await bcrypt.hash(user._id.toString(), 10);

                   const res = await request(server).get("/api/users/validate").query({code:code, email:"jake@test.com"});
                   expect(res.status).toBe(200);
                   expect(res.header[config.get("authTokenName")]).not.toBeNull();

...
describe("/api/habbits/notes", () => {
  ...
  describe("GET /:id", () => {
    ...
    it("Should return 404 if the user is not the owner of the Habbit Note", async () => {
      ...
      const res = await request(server)
        .get(`/api/habbits/notes/${habbitNote._id}`)
        .set(config.get("authTokenName"), token);
  ...
  describe("POST /", () => {
    ...
    it("Should save HabbitNote to database if it is valid and user is signed in", async () => {
      ...
      const habbitNote = await HabbitNote.findOne({
        userId: user._id,
        categoryId: "5fa74048d611ec711ecab478",
      });
  ...
  describe("PUT /:id", () => {
    ...
    const exec = async () => {
      return await request(server)
        .put(`/api/habbits/notes/${sampleHabbitNote._id}`)
        .set(config.get("authTokenName"), token)
        .send(habbitNote);
    };
    ...
    it("Should return 404 if Object Id is not found", async () => {
      const res = await request(server)
        .put(`/api/habbits/notes/${mongoose.Types.ObjectId()}`)
        .set(config.get("authTokenName"), token)
        .send(habbitNote);
      expect(res.status).toBe(404);
    });
  ...
  describe("DELETE /:id", () => {
    ...
    const exec = async () => {
      return await request(server)
        .delete(`/api/habbits/notes/${sampleHabbitNote._id}`)
        .set(config.get("authTokenName"), token);
    };

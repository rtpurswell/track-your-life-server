...
router.get("/",[auth,validateDates], async(req,res)=> {
    ...
    const habbitRecords = await HabbitRecord.find({userId:user._id, date: {$gte:req.query.startDate, $lte:req.query.endDate}});
...
router.delete("/:id",[validateObjectId,auth], async(req,res)=> {
    ...
    const habbitRecord = await HabbitRecord.deleteOne({userId:user._id, _id:req.params.id});
    res.send(habbitRecord);
});

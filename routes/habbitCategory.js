...
router.delete("/:id",[auth, validateObjectId], async(req,res)=> {
    const {user} = req;
    if(!req.body.replacedBy) return res.status(400).send("Must specify 'replacedBy' in request body to update existing habbits in this category");
    ...
    const count = await HabbitCategory.find({userId:user._id}).countDocuments();
    if(count<2) return res.status(400).send("You can not delete your only category.");
    
    const habbitCategory = await HabbitCategory.deleteOne({userId:user._id,_id:req.params.id});
    const habbits =  await Habbit.updateMany({userId:user._id,categoryId:req.params.id},{categoryId:req.body.replacedBy});
    const notes = await HabbitNote.updateMany({userId:user._id,categoryId:req.params.id},{categoryId:req.body.replacedBy});

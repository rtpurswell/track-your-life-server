...
router.get("/",[auth,validateDates], async(req,res)=> {
    ...
    const habbitNotes = await HabbitNote.find({userId:user._id, date:{$gte:req.query.startDate, $lte:req.query.endDate}});
...
router.delete("/:id",[auth, validateObjectId], async(req,res)=> {
    ...
    const habbitNote = await HabbitNote.deleteOne({userId:user._id, _id:req.params.id});
    res.send(habbitNote);
